...
for org_f in tqdm(sample_f_list):
    org_desc_f = str(org_f).replace("new_description", "description")
    desc_f = str(org_f)
    ...
    if os.path.exists(desc_f) and os.path.exists(desc_en_f):
        org_desc_en_txt = open(org_desc_f, 'r').read()
        desc_txt = open(desc_f, 'r').read()
        desc_en_txt = open(desc_en_f, 'r').read()
        description_data_list.append([f_path, org_desc_en_txt, desc_txt, desc_en_txt])
