...
if __name__ == '__main__':
    ...
    # 写入标题
    # ------------------
    for col, column in enumerate(["图片路径", "以前文案", "中文文案", "英文文案"]):
        sheet.write(0, col, column)

    # -------------------
    # 数据
    # -------------------
    input_file_name = '/Users/jiahong/Downloads/ ai宠物旅行景点名(36)-20231221.xlsx'
    ...
    for index, row in df.iterrows():
        country = row["国家"]
        place_name = row["地区"]
        country_en = get_gpt35_result(prompt=config.place2en_prompt.format_map({"place_name": country})).strip(".")
        place_name_en = get_gpt35_result(prompt=config.place2en_prompt.format_map({"place_name": place_name})).strip(
            ".")
    ...
    workbook.save("ai宠物国家景点名中英文20231221.xls")
