...
state_prompt = pet_world_str + (
    ...
    "输出你状态时候，考虑下你的朋友{friend_name}当前的状态。比如：\n"
    ...
    "注意：\n"
    "1.主人不可能跟宠物在虚拟世界中玩耍的。\n"
    "2.在规划状态和下一步计划时候，不要出现跟朋友进行玩耍的情况。\n"
    "3.输出状态时候，不要出现你去找你的朋友情况，永远不要自己去帮助你的朋友，而是叫你的主人去告诉你朋友的主人来帮助你的朋友。\n\n"


    "现在按如下格式输出：\n"
    "饱腹感: (当前你的肚子饥饿程度，输出比当前状态中的饱腹感低5分的分数，只输出数值，数值范围为：0~100)\n"
    "思考: (输出当前你在想什么，字数不超过30。你的思考要一句当前的这状态和今天的计划，但是要发挥合理的想象和创造力，思考一些除此之外的随机事件)\n"
    "状态: (当前时间为:{curr_time}，根据当前时间，生成一个当前你在干什么的状态，这个状态可以是开心的或者不好的遭遇，生成的内容要具有多样性，描述可以适当夸张，可以发挥合理想象，你的想象也要包含对身边环境、对交互的朋友，主人，外人等进行合理想象，并依据对朋友、主人、外人等的想象进一步发挥想象。想象中可以包含一些不好的突发事件，比如在游玩过程中受伤等，同时考虑你的朋友{friend_name}的当前状态来输出你当前的状态。字数不超过50。)\n"
