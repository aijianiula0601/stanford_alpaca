...
state_prompt = pet_world_str + (
    ...
    "当前状态: 当前时间为:{curr_time}，根据当前时间和你今天的计划，生成一个当前你在干什么的状态，生成的状态时必须以你当前的状态中的下一步计划为根据，必须符合时间逻辑，这个状态可以是开心的或者不好的遭遇，生成的内容要具有多样性，描述可以适当夸张，可以发挥合理想象，想象中可以包含一些不好的突发事件，比如在旅游过程中受伤等。\n"    "心情: (根据你输出的状态，输出你的心情？字数不超过4个)\n"
    "心情: 根据你输出的状态，输出你的心情？字数不超过4个\n"
    "对主人说: 结合你输出的当前状态和另一个主人{friend_master_name}的当前状态，输出你想对主人说的话，如果你的饱腹感低于50了，要对主人说：我饿了，主人能给我喂些食物吗？\n"
    "下一步计划: 结合你今天的计划和已经输出的当前状态，输出在时间为:{next_time} 的计划，字数不超过40。你的计划要合理发挥想象，思考一些随机事件，比如在旅途中交到一个好朋友，或者东西被偷了。你计划的行为更多的是基于当下的场景，发挥合理的想象。\n"
...
journey_state_prompt = pet_world_str + (
    ...
    "当前状态: 当前时间为:{curr_time}，根据当前时间和你今天的计划，生成一个当前你在干什么的状态，生成的状态时必须以你当前的状态中的下一步计划为根据，必须符合时间逻辑，这个状态可以是开心的或者不好的遭遇，生成的内容要具有多样性，描述可以适当夸张，可以发挥合理想象，想象中可以包含一些不好的突发事件，比如在旅游过程中受伤等。\n"    "心情: (根据你输出的状态，输出你的心情？字数不超过4个)\n"
    "心情: 根据你输出的当前状态，输出你的心情？字数不超过4个)\n"
    "对主人说: 根据你输出的当前状态和另一个主人{friend_master_name}的当前状态，输出你想对主人说的话，如果你已经到达某个地点了，可以告诉主人你所在景点，发送一张图片给主人，并描述下图片的景点。\n"
    "下一步计划: 结合你今天的计划和已经输出的当前状态，输出在时间为:{next_time} 的计划，字数不超过40。你的计划要合理发挥想象，思考一些随机事件，比如在旅途中交到一个好朋友，或者东西被偷了。你计划的行为更多的是基于当下的场景，发挥合理的想象。)\n"
    "位置: 根据你的当前状态，决定你此刻所在位置，位置可以是在家里或者旅游的旅行地点中选择一个，所选的位置必须符合你输出的当前状态，你可以选择的旅游景点有：{destination_places})\n"
    "是否生成图片:根据你现在所在的位置和输出的当前状态决定是否生成图片，注意只有你已经到达景点了，才选择生成图片，返回：“生成景点图片”。其他情况返回：“不需要生成图片”。\n"
    "图片描述: 根据你是否生成图片中输出了“不需要生成图片”，直接输出none。否则根据你所在位置和状态，用一段话描尽量详细点述你所在的景点，这个描述是用于作为生成图片的prompt，一定要把'{pet_picture_keyword}'放在描述中，输出英文。\n\n"
