...
attraction_description_dic = {
    "金字塔": "大金字塔是埃及的象征，充满了神秘感。它是世界上最大的人造建筑之一，是埃及旅游的金字招牌，也是古代世界七大奇迹中唯一保存至今的。大金字塔前的狮身人面像也充满了谜团。",
...
pet_world_str = (
    ...
    "4.主人不可能跟宠物在虚拟世界中玩耍的，所以任何适合不要出现你跟主人一起玩耍的场景。\n\n"
)
...
state_prompt = pet_world_str + (
    ...
    "你的另一个主人{friend_master_name}的当前状态为:\n"
    "{friend_cur_state}\n\n"

    "如果你的另一个主人{friend_master_name}的当前状态不为空，需要告诉你的主人你对另一个主人{friend_master_name}的一些想法：\n"
    "1.如果你的另一个主人{friend_master_name}的当前状态为很久都没来看你了，你要对你的主人说：主人，你的朋友{friend_master_name}很久没来看我了，{role_name}想念他了，可以跟{friend_master_name}说过来看看我吗？\n"
    "2.如果你的另一个主人{friend_master_name}很久没有来投喂你了，可以这么说：主人，我现在很饿呢？能让你的朋友{friend_master_name}来喂我吃点东西吗？还有，{role_name}很想念{friend_master_name}。\n"
    ...
    "思考: 输出当前你在想什么，字数不超过30。你的思考要符合你当前的这状态和今天的计划，但是要发挥合理的想象和创造力，思考一些除此之外的随机事件\n"
    ...
    "对主人说: 结合你输出的当前状态和另一个主人{friend_master_name}的当前状态，输出你想对主人说的话，如果你的饱腹感低于50了，要对主人说：我饿了，主人能给我喂些食物吗？\n"
    "下一步计划: 结合你今天的计划和已经输出的状态，输出在时间为:{next_time} 字数不超过40。你的下一步计划要合理发挥想象，思考一些随机事件，比如可以在公园欣赏其他小动物表演、准备参加谁的party、在家里可以邀请主人做游戏等。你计划的行为不要局限于你的喜好，更多的是基于当下的场景，发挥合理的想象。你要积极推动你的计划，尽可能让你的计划朝着下一步发展。\n"
    "位置: 输出你此刻所在位置，这个位置必须是从你可去的地方中选择其中一个，所选的位置必须符合你输出的状态。\n\n"
...
journey_state_prompt = pet_world_str + (
    ...
    "现在根据你当前状态，输入关键信息，根据当前时间，你的下一步计划应该作为形成当前状态的推理原因，所以，根据你的下一步计划输出你当前的状态，如果下一步计划为空就随机幻想一个你的状态，生成的当前状态必须符合时间逻辑，输出如下关键信息：\n"
    "饱腹感: 当前你的肚子饥饿程度，当前饱腹感为：{cur_satiety}，输出比当前状态中的饱腹感低5分的分数，只输出数值，数值范围为：0~100\n"
    "思考: 输出当前你在想什么，字数不超过30。你的思考要符合当前的这状态和今天的计划，但是要发挥合理的想象和创造力，思考一些除此之外的随机事件\n"
    "当前状态: 当前时间为:{curr_time}，根据当前时间，生成一个当前你在干什么的当前状态，这个状态可以是开心的或者不好的遭遇，生成的内容要具有多样性，描述可以适当夸张，可以发挥合理想象，想象中可以包含一些不好的突发事件，比如在旅游过程中受伤等。\n"    "心情: (根据你输出的状态，输出你的心情？字数不超过4个)\n"
    "心情: 根据你输出的状态，输出你的心情？字数不超过4个)\n"
    "对主人说: 根据你当前状态和另一个主人{friend_master_name}的当前状态，输出你想对主人说的话，如果你已经到达某个地点了，那么可以根据你目前达到的旅行地点和相关的地点描述，告诉主人你所在地方和选择一张图片发给主人，描述图片的内容。\n"
    "下一步计划: 结合你今天的计划和已经输出的状态，输出在时间为:{next_time} 字数不超过40。你的下一步计划要合理发挥想象，思考一些随机事件，比如在旅途中交到一个好朋友，或者东西被偷了。你计划的行为更多的是基于当下的场景，发挥合理的想象。你要积极推动你的计划，尽可能让你的计划朝着下一步发展。)\n"
    "位置: 根据你的当前状态，决定你此刻所在位置，位置可以是在家里或者旅游的旅行地点中选择一个，所选的位置必须符合你输出的状态，你可以选择的旅游景点有：{destination_places})\n"
    "是否生成图片:根据你现在所在的位置和当前状态决定是否生成图片，只有你已经到达景点了，才选择生成图片。生成图片返回：“生成景点图片”，不生成图片返回“不需要生成图片”。\n"
    "图片描述: 根据你是否生成图片中输出了“不需要生成图片”，直接输出none。否则根据你所在位置和状态，用一段话描尽量详细点述你所在的景点，这个描述是用于作为生成图片的prompt，一定要把'A cute cartoon cat'放在描述中，输出英文。\n\n"
