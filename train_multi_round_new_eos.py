...
from conversation_new_eos import get_default_conv_template, SeparatorStyle
...
def preprocess(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        #         rank0_print(
        #             f"{index} WARNING: tokenization mismatch " f"{cur_len} vs. {total_len}"
        #         )
        # index += 1
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        list_data_dict = json.load(open(data_path, "r"))#[:1000]
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sources = [sources]
        data_dict = preprocess([e for e in sources], self.tokenizer)
        if isinstance(i, int):
            data_dict = dict(
