...
def llama_respond(message_list, role_dict, temperature=0.6):
    ...
    response = requests.post("http://202.168.100.169:805/api/llama", data=request_data)
...
def llama_respond_emotion(message_list, role_dict, temperature=0.6):
    '''message-list第一个数值是背景，
    后面需要在role_dict里要做好配置，我最后会回复role_dict['assistant']角色的答案;
    role_dict_real用于映射history里的内容'''
    background = message_list[0]["content"]
    ...
    response = requests.post("http://202.168.100.169:803/api/llama", data=request_data)
    json_data = json.loads(response.text)
    text_respond = json_data["result"]
    # print("REAL Respond", text_respond)
    text_respond = text_respond.strip().split(role_dict['user'] + ": ")[0]
...
def llama_respond_context(message_list, role_dict, temperature=0.6):
    '''message-list第一个数值是背景，
    后面需要在role_dict里要做好配置，我最后会回复role_dict['assistant']角色的答案;
    role_dict_real用于映射history里的内容'''
    CONTEXT_PROMPT_DICT = {
        "conversion_bak": (
            # "The following is a chat message between {role_a} and {role_b}. Question and answer, forbid the output of multiple rounds. {background}\n\n"
            "{background}. The following is a chat message between {role_a} and {role_b} using English Language. Question and answer, forbid the output of multiple rounds. Current conversation."
            "{history}"
        ),
    "conversion": (
        # "The following is a chat message between {role_a} and {role_b}. Question and answer, forbid the output of multiple rounds. {background}\n\n"
    ...
    response = requests.post("http://202.168.100.169:807/api/llama", data=request_data)
    json_data = json.loads(response.text)
    text_respond = json_data["result"]
    # print("REAL Respond\n", text_respond)
    # text_respond = text_respond.strip().split(role_dict['user']+": ")[0]
