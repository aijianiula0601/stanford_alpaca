...
# ------------------------------------------------------------
# open-platypus指令性数据，提升模型逻辑能力
# ------------------------------------------------------------
data_base_dir = '/mnt/cephfs/hjh/common_dataset/nlp/instruction/Open-Platypus'
f_p = f"{data_base_dir}/train-00000-of-00001-5b226e5ae97bf4b1_qas.txt"
open_platypus_data_list = [json.loads(line) for line in open(f_p).readlines()]
random.shuffle(open_platypus_data_list)
dataset_name = OPEN_PLATYPUS_DATASET_NAME
for example in open_platypus_data_list:
    example[DATASET_KEY] = dataset_name
    example[MASK_HEAD_KEY] = True
    example[MASK_QUESTION_KEY] = True
    example[MASK_EXCEPT_LAST_ANSWER] = False

print(f"dataset:{dataset_name},all_n:{len(open_platypus_data_list)}")
...
other_data = soda_data_list + persona_chat_data + databricks_dolly_15k_data_list + cnn_dailymail2qas_data_list + openorca_data_list + gpt_roleplay_realm_data_list + open_platypus_data_list
