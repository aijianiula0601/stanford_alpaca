...
base_dir="/mnt/cephfs/hjh/train_record/nlp/stanford_alpaca/vicuna-7b/ft2_v15/v6_v4"
...
if [ ! -f "${data_json}" ]; then
  ...
  python ${curdir}/prepare_data_v4.py ${org_f} ${data_json}
  echo "------------------------------------------------------------------------------"
fi
...
torchrun --nproc_per_node=8 --master_port=${your_random_port} test_models/vicuna-7b/train_mask_control_in_data.py \
    ...
    --fp16 True \
    --process_name "vicuna-7b-v15_v6_v4" \
    --lazy_load
