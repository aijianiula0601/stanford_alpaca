...
pdf = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
sys.path.append(pdf)
...
if __name__ == '__main__':
    ...
    org_f = f"{base_dir}/bigolive_robot_chat_history.for_train.20230804-20230808.starter_user.v2.en.txt"
    save_f = f"{base_dir}/bigolive_robot_chat_history.for_train.20230804-20230808.starter_user.v2.en_gpt4to_colloquial.txt"

    exp_tag_list = ['add_gift_prompt', 'prompt_optimize']
    ...
    with open(save_f, 'a', buffering=1) as fw:
        for line in tqdm(org_data_list):
            try:
                ...
                if example['exp_tag'] not in exp_tag_list:
                    continue
