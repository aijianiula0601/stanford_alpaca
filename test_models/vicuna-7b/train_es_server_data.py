...
class DataArguments:
    ...
    es_server_url: str = field(default='')
    es_user_name: str = field(default='')
...
def _preprocess_example(conversation_dic: Dict, tokenizer: transformers.PreTrainedTokenizer, token_max_len: int,
    ...
    for i in range(turn_n):
        cur_turn_qa = conversation_dic[QAS_KEY][i]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, token_max_len: int, mask_head: bool,
                 mask_question: bool, mask_except_last_answer: bool, es_index_name: str, es_server_url: str,
                 es_user_name: str, es_password: str):
        ...
        self.es = Elasticsearch(es_server_url, http_auth=(es_user_name, es_password),
                                verify_certs=False)
        self.list_data_dict_len = index_count(self.es, es_index_name)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        input_ids, labels = _preprocess_example(get_data_id(self.es, self.es_index_name, id=i), self.tokenizer,
                                                self.token_max_len,
        ...
        while input_ids is None or labels is None:
            ...
            input_ids, labels = _preprocess_example(get_data_id(self.es, self.es_index_name, id=random_i),
                                                    self.tokenizer,
                                                    self.token_max_len,
                                                    self.mask_head, self.mask_question, self.mask_except_last_answer)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, token_max_len) -> Dict:
    ...
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer,
                                          ...
                                          es_index_name=data_args.es_index_name, es_server_url=data_args.es_server_url,
                                          es_user_name=data_args.es_user_name, es_password=data_args.es_password)
